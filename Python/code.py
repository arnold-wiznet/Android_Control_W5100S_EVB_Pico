    ...
    while i < len(useful_data):
        if i + 1 < len(useful_data):
            ...
            if current_tuple in tuple_to_value:
                ...
                if not value_switch_flag:
                    value_switch_flag = True
                    if first_value >= 0x8000:
                       first_value = - (first_value & 0x7fff)
                    first_value = useful_data[i]
                else:
                    value_switch_flag = False
                    if second_value >= 0x8000:
                       second_value = - (second_value & 0x7fff)
...
    ©¢*«       #     while True:
            #         now = time.monotonic()
            #         print(now - last_request_time)
            #         await asyncio.sleep(0.0)
            #         if (now - last_request_time >= 5):
            #             last_request_time = now
            #             # print(f"No requests in the last {reconnect_second} s. No client is talking to me")
            #             print("Wait for Check response here.")
            #             timeout_local = 20
            #             start_local = time.monotonic()
            #             while True:
            #                 try:
            #                     data, addr = udp_server.recvfrom(128)
            #                 except OSError:  # no data ready
            #                     await asyncio.sleep(0)  # give the loop a chance
            #                     continue
            ...
            #                         udp_server.sendto(reply, addr)
            #                     except OSError as e:
            #                         print("Send failed (socket non-blocking?):", e)
            ...
            #     await asyncio.gather(http_task(), ping_UDP_async(),sfa30_task())
